...
"""
Example decorest based async client to Swagger Petstore sample service.
...
class PetAPI(RestClient):
    ...
    async def add_pet(self, pet):
        """Add a new pet to the store."""

    @PUT('pet')
    @body('pet')
    async def update_pet(self, pet):
        """Update an existing pet."""

    @GET('pet/findByStatus')
    @on(200, lambda r: r.json())
    @on(HttpStatus.ANY, lambda r: r.raise_for_status())
    async def find_pet_by_status(self):
        """Find Pets by status."""

    @GET('pet/findByStatus')
    @accept('application/xml')
    @on(200, lambda r: ET.fromstring(r.text))
    async def find_pet_by_status_xml(self):
        """Find Pets by status."""

    @GET('pet/{pet_id}')
    async def find_pet_by_id(self, pet_id):
        """Find Pet by ID."""

    @POST('pet/{pet_id}')
    async def update_pet_by_id(self, pet_id):
        """Update a pet in the store with form data."""

    @DELETE('pet/{pet_id}')
    async def delete_pet(self, pet_id):
        """Delete a pet."""

    @POST('pet/{pet_id}/uploadImage')
    async def upload_pet_image(self, pet_id, image):
        """Upload an image."""
...
class UserAPI(RestClient):
    ...
    @body('user', lambda o: json.dumps(o))
    async def create_users_from_array(self, user):
        """Create list of users with given input array."""

    @POST('user/createWithList')
    @body('user', lambda o: json.dumps(o))
    async def create_users_from_list(self, user):
        """Create list of users with given input array."""
    ...
    @PUT('user/{username}')
    @body('user', lambda o: json.dumps(o))
    async def update_user(self, username, user):
        """Update user."""
